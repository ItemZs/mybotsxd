...
exports.run = (client, message, args) => {
  ...
  .addField(':warning: OOF :warning:', '`kick` cant working dm')
  ...
  if (reason.length < 1) return message.reply('Please write down the reason example : >kick @tag#2018 reason');
  if (message.mentions.users.size < 1) return message.reply('who ? example : >warn @tag#2018 reason').catch(console.error);

  if (!message.guild.member(user).kickable) return message.reply('I Cant Ban Staff');
